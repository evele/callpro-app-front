<template>
    <div class="button-page">
        <!-- botones dinamicos -->
        <CustomButton v-for="(button, index) in buttons" :key="index" :text="button.text" :value="button.value"
            :class="{ 'styled-button': true }" :active="activeButtonIndex === index" @click="setActiveButton(index)">
            <!-- Renderiza el componente SVG correspondiente -->
            <template #icon>
...
const buttons = [
    { text: 'ALL', value: 0, icon: ContactsSVG },// sumo los ramdomssss
    { text: 'Unassigned', value: Math.floor(Math.random() * 100), icon: GroupsSVG },//ramdom para los valores
