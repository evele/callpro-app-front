    ...
    <Dialog v-model:visible="visible" modal :draggable="false" :closable="false" class="w-full max-w-[700px] mx-4">
        ...
        <div class="flex justify-between w-full mb-7 mt-1">
            <div class="flex items-center gap-4">
                <Button v-show="show_verify_btn" @click="handle_verify_number" class="rounded-md h-9 pl-2 bg-white border-[#49454F] shadow-lg text-[#49454F] hover:bg-gray-200 disabled:bg-white" :disabled="disable_resend_btn">
                    <!-- <ProgressSpinner v-if="STTIsPending" class="w-5 h-5" strokeWidth="8" fill="transparent" animationDuration=".5s" aria-label="Deleting number" /> -->
                    <CheckSVG class="w-7 h-7" />
                    <span class="text-sm font-semibold tracking-wider leading-none pt-[2px]">Verify Number</span>
                </Button>

                <Button v-show="!show_verify_btn" @click="handle_resend_call" class="rounded-md h-9 bg-white border-[#49454F] shadow-lg text-[#49454F] hover:bg-gray-200 disabled:bg-white" :disabled="disable_resend_btn">
                    <!-- <ProgressSpinner v-if="STTIsPending" class="w-5 h-5" strokeWidth="8" fill="transparent" animationDuration=".5s" aria-label="Deleting number" /> -->
                    <CallOutSVG class="w-4 h-4" />
                    <span class="text-sm font-semibold tracking-wider leading-none pt-[2px]">Resend Call</span>
                </Button>

                <Button @click="handle_delete_number" class="rounded-md h-9 bg-white border-[#49454F] shadow-lg text-[#49454F] hover:bg-gray-200 disabled:bg-white" :disabled="disable_action_btn">
                    <!-- <ProgressSpinner v-if="STTIsPending" class="w-5 h-5" strokeWidth="8" fill="transparent" animationDuration=".5s" aria-label="Deleting number" /> -->
                    <TrashSVG class="w-5 h-5" />
                    <span class="text-sm font-semibold tracking-wider leading-none pt-[2px]">Delete Number</span>
                </Button>
            </div>
        </div>
            ...
            <table class="w-full text-gray-700 border border-gray-200 border-collapse">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    >
                        ...
                        <td class="text-center">
                            <VerifiedSVG v-if="number.status === CallerIDStatus.CONFIRMED" class="w-4 h-4 text-verified mx-auto" />
                            <PendingSVG v-if="number.status === CallerIDStatus.PENDING || number.status === CallerIDStatus.UNVERIFIED" class="w-4 h-4 text-pending mx-auto" />
                            <RejectedSVG v-if="number.status === CallerIDStatus.REJECTED" class="w-4 h-4 text-unverified mx-auto" />
                        </td>
                    </tr>
                    <tr v-else>
        ...
        <form @submit.prevent class="flex justify-end items-center gap-4 w-full mt-7">
            <div class="flex gap-4 items-center">
                <PhoneInput :model-value="new_number" @update:modelValue="(v: string) => new_number = v" class="w-[200px]"
                ...
                <Button @click="add_new_number" :class="[ add_is_pending ? 'w-32' : 'w-24']" class="bg-[#1D192B] border-none rounded-xl text-white hover:bg-[#322F35] disabled:bg-[#848287]"
                    :disabled="disabled_add_new_btn || add_is_pending">
            ...
            <footer class="flex justify-center w-full font-bold mt-[38px] mb-10">
                <Button @click="handle_close_modal" class="w-[300px]" :disabled="add_is_pending">
                    Save
                </Button>
            </footer>
    </Dialog>
...
<script setup lang="ts">
    ...
    const emit = defineEmits(['update:selected-caller-id'])
    ...
    const handle_delete_number = () => {
        ...
        console.log(selected_caller_id.value)
    }
